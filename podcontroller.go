...
func (c *PodController) handlePod(key string) error {
	...
	replicaSet, err := c.kubeclientset.AppsV1().ReplicaSets(namespace).Get(context.TODO(), ownerReferenceName, metav1.GetOptions{})
	if err != nil {
		// fmt.Println("PodController.go     : Couldn't find the replicaset, skipping")
		return nil
	}
	...
	deployment, err := c.kubeclientset.AppsV1().Deployments(namespace).Get(context.TODO(), ownerReferenceName, metav1.GetOptions{})
	if err != nil {
